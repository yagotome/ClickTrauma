...
public class PerguntaController {
		...
		try {
			...
			for (int i = 0; i < qtdResp; i++) {
				if (pergunta.getRespostas().get(i).getImg() != null
					...
					String extension = extensao(pergunta.getRespostas()
							.get(i).getImg().getOriginalFilename());
					String nome = "respostas\\" + pergunta.getRespostas()
							.get(i).getId() + extension;
	...
	public void remove(Long id, HttpServletResponse response) {
		...
		if (p.getImgUrl() != null) {
			...
			if (!arq.delete())
				throw new RuntimeException("Erro ao excluir imagem da pergunta " 
							+ p.getId() + ".");
		}
		for (Resposta r : p.getRespostas()) {
			if (r.getImgUrl() != null) {
				...
				if (!arq.delete())
					throw new RuntimeException("Erro ao excluir imagem da resposta " 
								+ r.getId() + ".");
			}
		}
		dao.remove(p);
